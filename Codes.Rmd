...
time.series.long$date <- as.Date(gsub("X", "", time.series.long$date), "%m.%d.%y") # change to date format
...
data_long <- data_long %>% 
    select(state, country, iso2, iso3, date, value, Lat,Long_, UID, code3, FIPS, Admin2, Population, everything()) %>% 
    arrange(country)
...
png(file = "overall_change.png")
overall_change <- data_long %>% 
    ggplot(aes(x = date, y = log.confirmed_cases)) +
    stat_summary(fun.y = mean,
                 geom = "line") + 
    theme_bw() +
    labs(x = "Date", y = "Log number of cases",
         title = "Overall change in time of log number of cases",
...
data_long$infection_rate <- data_long$confirmed_cases/data_long$total_population*100000 ## Calculate infection rate
